...
Процедура ФормируемЗаписиРасчетаОтпуска(Отказ, СтавкаНДФЛ)
	...
	Пока Выборка.Следующий() Цикл
		//анализируем месяц начала и месяц окончания (переход отпуска на другой расчетный период)
		ИнтервалыОтпуска = Новый Массив;
		Если Месяц(Выборка.ДатаНачала) <> Месяц(Выборка.ДатаОкончания) Тогда
			ИнтервалыОтпуска.Добавить(Новый Структура("ДатаНачала, ДатаОкончания",
			               Выборка.ДатаНачала, КонецМесяца(Выборка.ДатаНачала)));
			ИнтервалыОтпуска.Добавить(Новый Структура("ДатаНачала, ДатаОкончания",
			            НачалоМесяца(Выборка.ДатаОкончания),Выборка.ДатаОкончания));
		Иначе
			ИнтервалыОтпуска.Добавить(Новый Структура("ДатаНачала, ДатаОкончания",
...
Процедура НачислениеОтпуск()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ВКМ_НачислениеЗаработнойПлатыОтпуск.Сотрудник,
	|	ВКМ_НачислениеЗаработнойПлатыОтпуск.ДатаНачала,
	|	ВКМ_НачислениеЗаработнойПлатыОтпуск.ДатаОкончания
	|ПОМЕСТИТЬ ВТ_Сотрудники
	|ИЗ
	|	Документ.ВКМ_НачислениеЗаработнойПлаты.Отпуск КАК ВКМ_НачислениеЗаработнойПлатыОтпуск
	...
	|	ВТ_Сотрудники.ДатаНачала,
	|	ВТ_Сотрудники.ДатаОкончания,
	|	ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.НачисленноБаза,
	|	ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.ДнейОтработанноБаза
	|ИЗ
	|	ВТ_Сотрудники КАК ВТ_Сотрудники
